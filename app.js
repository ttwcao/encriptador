...
//função validar caracteres
function validaCaracteres(texareaValue) { 
    let pattern = new RegExp(/[A-Z\u00C0-\u00FF\-0-9\!@#\$%\^\&*\)\(+=._-]/gm,'');
        //"^([a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[-+_!@#$%^&*.,?]).+$" 
  
    if (pattern.test(texareaValue)) 
        return false; 
    else
        return true; 
    return; 
...
function criptografar(){
        ...
        document.getElementById('texto').placeholder='letras minúsculas e sem acento!';
        setTimeout(function(){
            location.reload();
        }, 2000);
    }
